...
const { sequelize, User, Project, Todo } = require("../models");
...
async function seed() {
  try {
    ...
    // bikin project dummy punya user1
    const [project1] = await Project.findOrCreate({
      where: { name: "Tugas Kuliah", user_id: user1.id },
      defaults: { description: "Kumpulan tugas semester ini" },
    });

    console.log("Project dummy siap:", project1.name);
